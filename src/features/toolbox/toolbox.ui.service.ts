...
export class ToolboxUIService {
    ...
    public async createToolModal(tool: ITool): Promise<void> {
    ...
    private getRemoveAllEmptyLinesHTML(tool: ITool, webview: vscode.Webview): string {
        return `<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    private setupDashboardMessageHandlers(): void {
    ...
    private setupToolMessageHandlers(panel: vscode.WebviewPanel, tool: ITool): void {
            ...
            switch (message.command) {
                ...
                case 'confirmExecution':
                    ...
                    const confirmed = await vscode.window.showWarningMessage(
                        'ATENÇÃO: Esta ação removerá TODAS as linhas vazias dos arquivos selecionados e não pode ser desfeita.\n\nDeseja continuar?',
                        { modal: true },
                        'Sim, executar'
                    );
        ...
        try {
            for (const folder of workspaceFolders) {
                ...
                const folderFiles = await vscode.workspace.findFiles(
                    '**/*',
                    '**/node_modules/**,**/.git/**,**/out/**,**/dist/**,**/build/**,**/.vscode/**,**/venv/**,**/.venv/**,**/env/**,**/__pycache__/**,**/.pytest_cache/**,**/coverage/**,**/.nyc_output/**'
                );
    ...
    private organizeFilesByStructure(files: vscode.Uri[], basePath: string): any[] {
        const structure: any[] = [];
